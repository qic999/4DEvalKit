...
class COSMOSDataset(BaseDataset):
    ...
    def __init__(
        self,
        dataset_name: str = "IffYuan/COSMOS",
        subset: Optional[str] = None,
        split: str = "train",
        instruct_following: Optional[str] = None,
        task_name: str = "COSMOS",
    ...
    def load_dataset(self) -> Any:
        """Load COSMOS dataset from HuggingFace"""
    ...
    def prepare_dataset(self, dataset: Any) -> List[Dict[str, Any]]:
        ...
        for idx, sample in enumerate(dataset):
            ...
            sample_id = sample["id"]
            domain = sample_id.split("_")[0] if "_" in sample_id else "unknown"

            prepared.append({
                "question": full_prompt+ "Please answer with the letter of your choice (A, B, C, or D)."+self.instruct_following,
                "answer": answer,
                "video": [video_frames],
                "metadata": {
                    "idx": idx,
    ...
    def extract_answer_from_text(text: str) -> Optional[str]:
        ...
        answer_patterns = [
            r'(?:Answer|Answer)[:\s]*\(([A-Za-z])\)',  # Answer: (A)
        ...
        # Strategy 2: Search for all occurrences of (A), (B), (C), (D), etc.
        all_matches = list(re.finditer(r'\(([A-Za-z])\)', text, re.IGNORECASE))
        if all_matches:
            return all_matches[-1].group(1).upper()

        # Strategy 3: Direct search for letter A-Z (word boundary)
    ...
    def evaluate_results(
        ...
        logger.info("Evaluating COSMOS Predictions...")
        results = []
        ...
        for sample, output in tqdm(zip(prepared_dataset, raw_outputs), total=len(prepared_dataset), desc="Evaluation"):
            try:
                ...
                logger.error(f"Error evaluating sample {sample.get('metadata', {}).get('question_id')}: {e}")
                results.append({
    ...
    def compute_statistics(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # Logging
        logger.info("\n" + "="*40)
        logger.info(f"COSMOS Statistics (Total: {count})")
        logger.info("="*40)
